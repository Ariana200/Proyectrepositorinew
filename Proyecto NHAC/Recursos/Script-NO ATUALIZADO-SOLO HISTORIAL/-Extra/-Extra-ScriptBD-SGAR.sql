...
VALUES('0941275786', 'Allan', 'Joel', 'Castro', 'Mora', 'Masculino', GETDATE(), '0992015890'); 

INSERT INTO PERSONA (CEDULA,NOMBRE_1,NOMBRE_2,APELLIDO_1,APELLIDO_2,SEXO,FECHA_NAC,TELEFONO)
VALUES('0914201033', 'ariana', 'Lisbeth', 'Bernal', 'Veliz', 'Femenino', GETDATE(), '0992015890'); 

INSERT INTO PERSONA (CEDULA,NOMBRE_1,NOMBRE_2,APELLIDO_1,APELLIDO_2,SEXO,FECHA_NAC,TELEFONO)
VALUES('0941275786', 'Allan', 'joshua', 'mendez', 'Vega', 'Masculino', GETDATE(), '0925478691'); 


INSERT INTO USUARIO(NOMBRE_USUARIO, CORREO, CONTRASENIA)
VALUES('AllanM','allanmendezvega@gmail.com','12345');
INSERT INTO USUARIO(NOMBRE_USUARIO, CORREO, CONTRASENIA)
...
-- -------------------------------------- PROCEDIMIENTO PARA EDITAR AMBULANCIAS -------------------------------
...
-- -------------------------------------- PROCEDIMIENTO PARA BORRAR AMBULANCIAS -------------------------------
...
AS 
	BEGIN
		...
		IF NOT EXISTS (SELECT 1 FROM PERSONA WHERE CEDULA = @cedula) BEGIN
			...
			INSERT INTO PERSONA (CEDULA, NOMBRE_1, NOMBRE_2, APELLIDO_1, APELLIDO_2, SEXO, FECHA_NAC, TELEFONO)
			VALUES				(@cedula, @nombre_1, @nombre_2, @apellido_1, @apellido_2, @sexo, @fecha_nac, @telefono)
			SET @id_persona = (SELECT ID_PERSONA FROM PERSONA WHERE CEDULA = @cedula)
			--CONDUCTOR
			INSERT INTO CONDUCTOR	(ID_PERSONA, FECHA_CONTRATO)
			VALUES					(@id_persona, @fecha_contrato)
			-- DATOS GUARDADOS
...
---------------------------------------------------------* CONDUCTOR LISTAR DATOS POR CEDULA NOMBRE DISPONIBILIDAD
CREATE PROCEDURE sp_conductor_listarDatosPor_Cedula_Nombre_Disponibilidad
...
AS
	...
	BEGIN
		set @str_where	= N' WHERE C.ID_ESTADO <> 0 '
		set @str_and	= N' AND '
		...
		IF (@cedula_nombre IS NOT NULL ) BEGIN 
			IF (ISNUMERIC(@cedula_nombre) = 1) BEGIN
				SET @str_where = CONCAT(@str_where, @str_and, @str_cedula)
			END ELSE BEGIN
				SET @str_where = CONCAT(@str_where, @str_and, @str_nombre)
			END
			...
			END
		END ELSE BEGIN
			SET @str_where = CONCAT(@str_where, @str_and, @str_disponibilidad)
			set @str_and = N' '
		END
...
AS 
	BEGIN
		...
		IF EXISTS (SELECT 1 FROM PERSONA WHERE CEDULA = @cedula) BEGIN
			...
			SET @id_persona = (SELECT ID_PERSONA FROM PERSONA WHERE CEDULA = @cedula)
			-- CONDUCTOR
			SET @id_disponibilidad = (SELECT ID_DISPONIBILIDAD FROM DISPONIBILIDAD WHERE NOMBRE_DISPONIBILIDAD = @disponibilidad)
			SET	@id_estado = (SELECT ID_ESTADO FROM ESTADO WHERE NOMBRE_ESTADO = @estado)
...
---------------------------------------------------------* CONDUCTOR ELIMINAR DATOS
CREATE PROCEDURE sp_conductor_eliminarDatos
	@id_conductor INT
	WITH ENCRYPTION
AS
	BEGIN
		...
		IF NOT EXISTS (SELECT 1 FROM PERSONA WHERE CEDULA = @cedula) BEGIN
			IF NOT EXISTS (SELECT 1 FROM USUARIO WHERE CORREO = @correo) BEGIN
				IF NOT EXISTS (SELECT 1 FROM USUARIO WHERE NOMBRE_USUARIO = @nombre_usuario) BEGIN
					...
					INSERT INTO USUARIO (NOMBRE_USUARIO, CORREO, CONTRASENIA)
					VALUES				(@nombre_usuario, @correo, @contrasenia)
					SET @id_usuario = (SELECT ID_USUARIO FROM USUARIO WHERE NOMBRE_USUARIO = @nombre_usuario)
					-- SECRETARIA
					INSERT INTO SECRETARIA (ID_PERSONA, ID_USUARIO, FECHA_CONTRATO)
					VALUES					(@id_persona, @id_usuario, @fecha_contrato)
					-- DATOS GUARDADOS
...
---------------------------------------------------------* SECRETARIA LISTAR DATOS POR CEDULA NOMBRE DISPONIBILIDAD
CREATE PROCEDURE sp_secretaria_listarDatosPor_Cedula_Nombre_Estado
	@cedula_nombre	VARCHAR (50),
	@estado	VARCHAR (1)
	WITH ENCRYPTION
AS
	...
	declare @str_cedula varchar(max)
	declare @str_nombre varchar(max)
	declare @str_estado varchar(max)

	BEGIN
		set @str_where	= N' WHERE S.ID_ESTADO <> 0 AND  U.ID_ESTADO <> 0 '
		set @str_and	= N' AND '
		set @str_cedula = N' P.CEDULA = '+ @cedula_nombre
		set @str_nombre = N' (
			CONCAT(P.NOMBRE_1, P.NOMBRE_2, P.APELLIDO_1, P.APELLIDO_2)	like ''%'+ @cedula_nombre +'%'' OR 
			...
			CONCAT(P.NOMBRE_1, P.APELLIDO_2)							like ''%'+ @cedula_nombre +'%'' OR 
			CONCAT(P.NOMBRE_2, P.APELLIDO_2)							like ''%'+ @cedula_nombre +'%'' )'
		set @str_estado	= N' C.ID_ESTADO = ' + @estado
		...
		IF (@cedula_nombre IS NOT NULL ) BEGIN 
			...
			IF (@estado IS NOT NULL) BEGIN
				set @str_and = CONCAT(@str_and, @str_estado)
			END ELSE BEGIN
				set @str_and = N' '
			END
		END ELSE BEGIN
			SET @str_where = CONCAT(@str_where, @str_and, @str_estado)
			set @str_and = N' '
		END
		set @str_consulta = CONCAT(@str_select, @str_where, @str_and)
		PRINT @str_consulta
...
CREATE PROCEDURE sp_secretaria_buscarDatosPor_Id
...
AS
	BEGIN
		...
		INNER JOIN USUARIO			AS U ON S.ID_USUARIO		= U.ID_USUARIO
		INNER JOIN ESTADO			AS E ON S.ID_ESTADO			= E.ID_ESTADO
		AND ID_SECRETARIA = @dato_idSecretaria 
	END;
GO
...
AS
	BEGIN
		...
		SET @existe_correo = (SELECT COUNT(*) FROM USUARIO AS U WHERE U.CORREO = @correo AND U.NOMBRE_USUARIO != @nombre_usuario)

		IF (@existe_correo = 0) BEGIN 

			SET @id_persona = (SELECT S.ID_PERSONA FROM SECRETARIA AS S INNER JOIN PERSONA P ON P.ID_PERSONA = S.ID_PERSONA WHERE S.ID_SECRETARIA = @id_secretaria);
			SET @id_usuario = (SELECT S.ID_USUARIO FROM SECRETARIA AS S INNER JOIN USUARIO AS U ON U.ID_USUARIO = S.ID_USUARIO WHERE S.ID_SECRETARIA = @id_secretaria);
			SET @id_estado = (SELECT ID_ESTADO FROM ESTADO WHERE NOMBRE_ESTADO = @estado)
...
---------------------------------------------------------* SECRETARIA ELIMINAR DATOS
CREATE PROCEDURE sp_secretaria_eliminarDatos
	@id_secretaria INT
	WITH ENCRYPTION
AS
	BEGIN
		DECLARE @id_usuario INT
		SET @id_usuario = (SELECT ID_USUARIO FROM SECRETARIA WHERE ID_SECRETARIA = @id_secretaria)
...
CREATE PROCEDURE sp_cliente_buscarDatosPorIdCedula
	@dato varchar(75),
	@tipo INT
	WITH ENCRYPTION
AS
	declare @str_select varchar(max) = N'Select client.ID_CLIENTE, client.ID_ESTADO, person.CEDULA, client.ID_HOSPITAL, person.NOMBRE_1,
		person.NOMBRE_2, person.APELLIDO_1, person.APELLIDO_2, person.FECHA_NAC, person.SEXO, usur.CORREO,
		person.TELEFONO, usur.CONTRASENIA, usur.NOMBRE_USUARIO
	...
	if(@tipo = 1) 
		BEGIN
			 ...
			 if(@dato='')begin
				set @str_where = N' where client.ID_CLIENTE = '' '+ CONVERT(varchar(4), @dato) +' '''
			end
			else set @str_where = N' where client.ID_CLIENTE = ' + CONVERT(varchar(4), @dato)
		END
	else if(@tipo = 2) 
		BEGIN
			if(@dato='')begin
 		...
 		END
set @str_consulta = CONCAT(@str_select, @str_where, ' AND client.ID_ESTADO != 0')
PRINT @str_consulta
exec(@str_consulta)
GO

-------------------- PRCEDIMEINTO PARA ACTUALIZAR CLIENTE 
CREATE PROCEDURE sp_cliente_actualizarDatos
	-- Datos_persona
	@id_cliente		int,
	@id_estado		int,
	@nombre_1		VARCHAR (20),
...
AS 
	...
	declare @id_usuario int 
	declare @id_persona int
	set @id_usuario = (select clien.ID_USUARIO from cliente clien inner join USUARIO us on us.ID_USUARIO = clien.ID_USUARIO where clien.ID_CLIENTE = @id_cliente);
	set @id_persona = (select clien.ID_PERSONA from cliente clien inner join PERSONA pers on pers.ID_PERSONA = clien.ID_PERSONA where clien.ID_CLIENTE = @id_cliente);
	set @existecorreo = (select COUNT(*) from USUARIO where USUARIO.CORREO = @correo and USUARIO.NOMBRE_USUARIO != @nombre_usuario);
	if(@existecorreo =0) begin
		UPDATE PERSONA set NOMBRE_1 = @nombre_1, NOMBRE_2= @nombre_2, APELLIDO_1= @apellido_1, 
		APELLIDO_2 = @apellido_2, FECHA_NAC= @fecha_nac, SEXO = @sexo, TELEFONO= @telefono
		where ID_PERSONA= @id_persona;
		UPDATE USUARIO set CONTRASENIA = @contrasenia, ID_ESTADO = @id_estado where ID_USUARIO = @id_usuario; 
		UPDATE CLIENTE set ID_HOSPITAL = @idhospital, ID_ESTADO = @id_estado where ID_CLIENTE = @id_cliente;
	end
GO
...
CREATE PROCEDURE sp_cambiarcontrasenia
	@id_tipo int,
	@tipo varchar(35),
	@contraseniaantigua varchar(35),
	@contrasenianueva varchar(35)
as 
	declare @idusuario int 
	declare @str_consulta nvarchar(4000) 
	set @str_consulta = 'Select @idusuario = usur.ID_USUARIO from '+ @tipo +' tipo inner join  USUARIO usur 
			on tipo.ID_USUARIO= usur.ID_USUARIO where tipo.ID_'+@tipo+ ' = ' + CONVERT(varchar(4), @id_tipo)
	exec sp_executesql @str_consulta, N'@idusuario int out', @idusuario out;
	declare @verificarcontrasenia int 
	set @verificarcontrasenia = (select COUNT(*) from USUARIO usur where usur.CONTRASENIA = @contraseniaantigua and usur.ID_USUARIO = @idusuario);
	if(@verificarcontrasenia=1)BEGIN
		UPDATE USUARIO SET CONTRASENIA = @contrasenianueva WHERE ID_USUARIO= @idusuario;
	END
go

INSERT INTO PETICION (ID_CLIENTE, ID_TIPO_AMBULANCIA, N_AMBULANCIAS, PUNTO_ORIGEN, PUNTO_DESTINO, CONDICION, FECHA_REGISTRO, ID_ESTADO) 
VALUES (100, 3, 1, N'La Joya', N'Clínica Kennedy', N'En Progreso', CAST(N'2022-03-25T22:59:56.783' AS DateTime), 1); 
INSERT INTO PETICION (ID_CLIENTE, ID_TIPO_AMBULANCIA, N_AMBULANCIAS, PUNTO_ORIGEN, PUNTO_DESTINO, CONDICION, FECHA_REGISTRO, ID_ESTADO) 
VALUES (100, 1, 2, N'Hospital IESS Valdivia', N'Medicglobal', N'Asignada', CAST(N'2022-03-25T23:00:41.710' AS DateTime), 1);
INSERT INTO PETICION (ID_CLIENTE, ID_TIPO_AMBULANCIA, N_AMBULANCIAS, PUNTO_ORIGEN, PUNTO_DESTINO, CONDICION, FECHA_REGISTRO, ID_ESTADO) 
VALUES (100, 2, 1, N'Urdesa', N'Omnihospital', N'Asignada', CAST(N'2019-01-22T23:05:52.267' AS DateTime), 1);

INSERT INTO ASIGNACION_CABECERA ([ID_PETICION], [ID_SECRETARIA], [CONDICION], [FECHA_REGISTRO], [ID_ESTADO]) 
VALUES (101, 100, N'En Progreso', CAST(N'2022-03-25T23:02:42.903' AS DateTime), 1);
INSERT INTO ASIGNACION_CABECERA ([ID_PETICION], [ID_SECRETARIA], [CONDICION], [FECHA_REGISTRO], [ID_ESTADO]) 
VALUES (102, 100, N'Cumplida', CAST(N'2019-01-22T13:12:42.903' AS DateTime), 1);

INSERT INTO ASIGNACION_DETALLE ([ID_ASIGNACION_CABECERA], [ID_CONDUCTOR], [ID_AMBULANCIA], [FECHA_REGISTRO], [ID_ESTADO]) 
VALUES (100, 100, 101, CAST(N'2022-03-25T23:02:42.920' AS DateTime), 1);
INSERT INTO ASIGNACION_DETALLE ([ID_ASIGNACION_CABECERA], [ID_CONDUCTOR], [ID_AMBULANCIA], [FECHA_REGISTRO], [ID_ESTADO]) 
VALUES (100, 100, 101, CAST(N'2022-03-25T23:02:42.963' AS DateTime), 1);
INSERT INTO ASIGNACION_DETALLE ([ID_ASIGNACION_CABECERA], [ID_CONDUCTOR], [ID_AMBULANCIA], [FECHA_REGISTRO], [ID_ESTADO]) 
VALUES (101, 100, 101, CAST(N'2019-01-23T19:12:04.170' AS DateTime), 1);

GO
--------------------- procedimiento para eliminar asignaciones antiguas
...
--------------------- procedimiento para listar asignaciones antiguas
